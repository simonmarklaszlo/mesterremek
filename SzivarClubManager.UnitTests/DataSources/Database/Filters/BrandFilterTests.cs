...
public class BrandFilterTests : FilterTests<BrandFilter, Brand>
{
    private readonly BrandFilter Filter;
    ...
    public override void Filter_ShouldBeNotEmpty()
    {
        ...
        Assert.Equal("Brand", Filter.Name);
    }

    public override void Filter_ToString_ShouldReturnExpectedString()
    {
        const string expected = "Id=[1..10] Name=\"Brand\"";
    ...
    public override void Filter_Parse_ShouldRestoreValues()
    {
        ...
        IFilter<Brand> parsed = BrandFilter.Parse(filterString);

        Assert.IsType<BrandFilter>(parsed);

        BrandFilter brandFilter = (BrandFilter)parsed;

        Assert.Equal(1, brandFilter.MinId);
        Assert.Equal(10, brandFilter.MaxId);
        Assert.Equal("Brand", brandFilter.Name);
    }

    public override void Filter_Parse_ShouldParseCorrectly()
    {
        const string filterString = "Id=[1..10] Name=\"Brand\"";
