...
public sealed partial class UserPageDataViewModel : ViewModelBase
{
    [ObservableProperty] private User[] _currentPageData;
    ...
    private readonly IPageFactory<User> _factory;
    private readonly PageController<User> _controller;

    public UserPageDataViewModel(IPageFactory<User> factory, PageController<User> controller)
    {
        _factory = factory;
    ...
    [RelayCommand]
    private Task CopyRowValue(User? item) => Clipboard.CopyText(item?.ToCopiableString());
}